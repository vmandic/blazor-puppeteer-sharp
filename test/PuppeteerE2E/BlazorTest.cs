...
namespace PuppeteerE2E
{
  ...
  {
    private readonly static string host =
      Environment.GetEnvironmentVariable("test_target_host") ??
      "http://localhost:80";

    private readonly static string chromium_path =
      Environment.GetEnvironmentVariable("chromium_path") ?? "";

    [Fact]
    public async Task Should_navigate_click_naviagte_and_increment_counter()
    {
      Console.WriteLine("Starting test...");
      Console.WriteLine("ENV test_target_host: " + host);
      Console.WriteLine("ENV chromium_path: " + chromium_path);
      Console.WriteLine($"Browser file exists on path: {File.Exists(chromium_path)}");
      ...
      {
        Console.WriteLine("Chromium not found, downloading...");
